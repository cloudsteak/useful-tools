...
### 1. SSH kulcs generálása Windows-on

#### Előfeltételek: OpenSSH ellenőrzése

A Windows 10 (1809-es verzió) és újabb rendszereken az OpenSSH alapból elérhető. Ellenőrizzük:
...
```
Enter file in which to save the key (C:\Users\FelhasználóNév/.ssh/id_ed25519):
```
...
### 2. PuTTY konfigurálása Windows-on

> 💡 Ha az `ssh` parancs működik a PowerShell-ből, **nem feltétlenül kell PuTTY** – használhatod közvetlenül a beépített OpenSSH-t. A PuTTY akkor hasznos, ha grafikus felületet szeretnél, vagy a szerver csak `.ppk` formátumú kulcsot fogad el.

#### PuTTY letöltése

Töltsd le a hivatalos oldalról: **https://www.putty.org**

Keresd a `putty-64bit-X.XX-installer.msi` fájlt (a legújabb verziót) és telepítsd. A telepítő tartalmazza:
- **PuTTY** – a terminál kliens
- **PuTTYgen** – kulcsgenerátor és konverter
- **Pageant** – SSH kulcsügynök

---

#### PuTTYgen: OpenSSH kulcs konvertálása .ppk formátumra

A PuTTY saját kulcsformátumot (`.ppk`) használ. Ha már van OpenSSH kulcsod, konvertálnod kell.

**Lépések:**

1. Nyisd meg a **PuTTYgen**-t (Start menüből kereshető)

2. Kattints a **"Load"** gombra

3. A fájlböngészőben változtasd meg a szűrőt:
   - Alapból `PuTTY Private Key Files (*.ppk)` van kiválasztva
   - Változtasd át `All Files (*.*)` értékre
   - Navigálj ide: `C:\Users\FelhasználóNév\.ssh\`
   - Válaszd ki az `id_ed25519` fájlt (kiterjesztés nélküli!)

4. Ha van passphrase-ed, add meg

5. Sikeres betöltés után látnod kell a kulcs adatait a PuTTYgen ablakban

6. Kattints a **"Save private key"** gombra

7. Mentsd el pl. `id_ed25519.ppk` névvel valahova (pl. `C:\Users\FelhasználóNév\.ssh\`)

⚠️ **Figyelem:** A `.ppk` fájlt is ugyanolyan gondosan őrizd, mint az eredeti privát kulcsot!

---

#### PuTTY session beállítása

1. Nyisd meg a **PuTTY**-t

2. **Session beállítása:**
   - `Host Name (or IP address)`: írd be a szerver IP-jét vagy domain nevét (pl. `192.168.1.100` vagy `szerver.example.com`)
   - `Port`: alapból `22` (ne változtasd, hacsak a szerver adminisztrátora nem mondja)
   - `Connection type`: válaszd az `SSH`-t

3. **Kulcs megadása:**
   - Bal oldalt navigálj: `Connection` → `SSH` → `Auth` → `Credentials`
   - `Private key file for authentication` mezőnél kattints a `Browse` gombra
   - Keresd meg a `.ppk` fájlodat

4. **Felhasználónév beállítása (hogy ne kelljen mindig gépelni):**
   - Navigálj: `Connection` → `Data`
   - `Auto-login username` mezőbe írd be a felhasználóneved (pl. `ubuntu`, `root`, `admin`)

5. **Session mentése (hogy ne kelljen mindent újra beállítani):**
   - Menj vissza a `Session` fülre
   - `Saved Sessions` mezőbe adj egy nevet (pl. `Munka szerver`)
   - Kattints a `Save` gombra

6. **Kapcsolódás:** Kattints az `Open` gombra

---

#### Kapcsolódás parancssorból (PowerShell) – PuTTY nélkül

Ha az OpenSSH telepítve van, egyszerűen:

```powershell
# Alap kapcsolódás – felhasználónév@szerver_cím
ssh felhasznalonev@192.168.1.100

# Kapcsolódás specifikus kulccsal
ssh -i C:\Users\FelhasználóNév\.ssh\id_ed25519 felhasznalonev@szerver.example.com

# Kapcsolódás más porton (ha a szerver nem a 22-es portot használja)
...
#### 1. módszer: `ssh-copy-id` (legegyszerűbb – Mac/Linux)
...
#### 2. módszer: Manuális módszer (minden OS-en működik)

Ez a módszer akkor hasznos, ha az `ssh-copy-id` nem elérhető (pl. Windows PowerShell) vagy ha közvetlenül akarod szerkeszteni a fájlt.
...
Get-Content C:\Users\FelhasználóNév\.ssh\id_ed25519.pub
```
